...
const Omraa = ({ hundelchangeEtat, hundelCurrentview,routemanger }) => {
    ...
    const sendtodatabase = () => {
        ...
        let d =new Date()
        res.post('/logs.json',{log:d.toLocaleString()+" : "+localStorage.getItem('agent')+" passé une réservation Omraa Pour le client : "+NomCli})
        res.post('/caisse.json',{Num_cli:Math.round(Math.random()*100)*1985,date:date,agent:localStorage.getItem('agent'),Des:NomCli,TypeDepaiment,Moy_Paiement:ModeDePaiment,Montant:premiertranche?PremTranche:PrixVente,Remarque,type:"revenu"})
        res.post('/omraa.json',{agent:localStorage.getItem('agent'),datenow,NomCli,Vol,PrixVente,peixAch,TypeDepaiment,ModeDePaiment,Visa,Pass,Raba,HotelMakka,HotelMedina,Portablea,TypedeChambre,deposit:premiertranche?PremTranche:PrixVente})
        routemanger(0)
    }}
    return (
