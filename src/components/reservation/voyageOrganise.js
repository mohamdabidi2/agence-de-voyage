...
const sendtodatabase=()=>{
    if(verif()==false){
        return ;
                ...
                let d =new Date()
        res.post('/logs.json',{log:d.toLocaleString()+" : "+localStorage.getItem('agent')+" passé une réservation d'une voyage organis Pour le client : "+NomCli})
       
                res.post("/caisse.json",{Num_cli:Math.round(Math.random()*100)*1985,date:datenow,agent:localStorage.getItem('agent'),Des:NomCli,TypeDepaiment,Moy_Paiement:ModeDePaiment,Montant:premiertranche?PremTranche:PrixVente,Remarque,type:"revenu"})
                res.post("/voyageorg.json",{date:datenow,agent:localStorage.getItem('agent'),NomCli,destination,dateEntree,Datesortie,prixAch:peixAch,PrixVente,TypeDepaiment,ModeDePaiment,deposit:premiertranche?PremTranche:PrixVente,Remarque})
                routemanger(0)
            }
                ...
                <div className="groupinpiut">
                    ...
                    <div className="">
                        ...
                        <select className="inputdesign selectpaiment" onChange={(e)=>setModeDePaiment(e.target.value)} name="" id="">
                            <option disabled value="">choisi un mode de paiement</option>
